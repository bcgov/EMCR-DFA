...
namespace EMBC.ERA.Suppliers.Actions.Plugins
{
    ...
    {
        public string RpsContextCookie
        {
            get;
            set;
        }
    ...
    public class SpoAuthUtility
    {
            ...
            if (stsAuthToken != null)
            {
                if (DateTime.Now > stsAuthToken.Expires)
                {
                    ...
                    if (this.stsAuthToken != null)
                    {
                        ...
                        Cookie rpsContextCookie = new Cookie("RpsContextCookie", cookies.RpsContextCookie)
                        {
                            Path = "/",
                            Expires = this.stsAuthToken.Expires,
                            Secure = cookies.Host.Scheme.Equals("https"),
                        ...
                        cc.Add(this.spSiteUrl, rpsContextCookie);
        ...
        private CookieContainer GetRpsContextCookieInNewContainer(Uri spSiteUrl, string username, string password)
        {
            ...
            string endPoint = spSiteUrl.AbsoluteUri;
            ...
            //the cookies CookieContainer should now contain the needed RpsContextCookie
            return cookies;
        }
        ...
        private SPOAuthCookies GetSPOAuthCookies(SamlSecurityToken stsToken)
        {
            ...
            // first, get the RpsContextCookie
            request.CookieContainer = GetRpsContextCookieInNewContainer(this.spSiteUrl, this.username, this.password);

            ////DEBUG:
            //throw new System.Activities.InvalidWorkflowException("RpsContextCookie: '" + request.CookieContainer.GetCookies(this.spSiteUrl)["RpsContextCookie"].Value + "'.");

            // next, get the Sts cookies (FedAuth, rtFA)
            ...
            if (request != null && responseData != null)
            {
                ...
                {
                    ////DEBUG:
                    //throw new System.Activities.InvalidWorkflowException("Count of cookies after call to STS: '" + response.Cookies.Count.ToString() + "'.");
                    spoAuthCookies.RpsContextCookie = (response.Cookies["RpsContextCookie"] != null) ? response.Cookies["RpsContextCookie"].Value : null;
