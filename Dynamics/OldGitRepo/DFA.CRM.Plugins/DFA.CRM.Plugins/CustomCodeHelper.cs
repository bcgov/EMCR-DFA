...
namespace DFA.CRM.Plugins
{
    public static class CustomCodeHelper
    {
        ...
        {
            var query = new QueryExpression("era_systemconfig") { ColumnSet = new ColumnSet("era_value") };
            query.Criteria.AddCondition("statecode", ConditionOperator.Equal, 0); // Active
            query.Criteria.AddCondition("era_group", ConditionOperator.Equal, group);
        ...
        public static string GetSystemConfigurationsValueByGroupAndKey(IOrganizationService Service, ITracingService Tracing, string group, string key)
        {
            ...
            {
                query.Criteria.AddCondition("era_key", ConditionOperator.Equal, key);
            }
            query.AddOrder("modifiedon", OrderType.Descending); // Last Modified
            query.TopCount = 1;
            ...
            {
                result = results.Entities[0].GetAttributeValue<string>("era_value");
            }
            return result;
        }
