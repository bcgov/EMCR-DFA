...
namespace DFA.Portal.Custom.Actions
{
    class Helper
    {
        ...
        public static Entity RetrieveInvoiceRecord(IOrganizationService service, Guid invoiceID)
        {
            Entity result = null;
            ColumnSet cols = new ColumnSet(Invoice.Schema.INVOICE_NUMBER, Invoice.Schema.INVOICE_REF);

            QueryExpression exp = new QueryExpression(Invoice.ENTITY_NAME);
            exp.NoLock = true;
            exp.Criteria.AddCondition(Invoice.Schema.ID, ConditionOperator.Equal, invoiceID);
            exp.ColumnSet = cols;

            var coll = service.RetrieveMultiple(exp);

            if (coll != null && coll.Entities != null && coll.Entities.Count > 0)
                result = coll.Entities[0];
            else
                throw new InvalidPluginExecutionException(string.Format("Unable to find '{0}' Invoice record with ID= {0} ...", invoiceID.ToString()));

            return result;
        }
        public static Entity RetrieveApplicationRecord(IOrganizationService service, Guid applicationid)
        {
            Entity result = null;
            ColumnSet cols = new ColumnSet(AppApplication.ID,"dfa_name");

            QueryExpression exp = new QueryExpression(AppApplication.ENTITY_NAME);
            exp.NoLock = true;
            exp.Criteria.AddCondition(AppApplication.ID, ConditionOperator.Equal, applicationid);
            exp.ColumnSet = cols;
            ...
            if (coll != null && coll.Entities != null && coll.Entities.Count > 0)
                ...
                throw new InvalidPluginExecutionException(string.Format("Unable to find '{0}' applcationId record with ID= {0} ...", applicationid.ToString()));
