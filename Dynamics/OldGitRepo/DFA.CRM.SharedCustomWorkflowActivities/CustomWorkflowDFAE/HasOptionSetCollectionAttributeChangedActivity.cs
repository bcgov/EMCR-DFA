...
{
    public class HasOptionSetCollectionAttributeChangedActivity : CodeActivity
    {
        [RequiredArgument]
        ...
        /// </summary>
        OptionSetValueCollection OldValue;
        /// <summary>
        ...
        protected override void Execute(CodeActivityContext activityContext)
        {
            try
            {
                ...
                OldValue = TargetPreImage?.GetAttributeValue<OptionSetValueCollection>(schemaName);
                NewValue = Target.GetAttributeValue<OptionSetValueCollection>(schemaName);
                ...
                var valuesAreEqual = OldValue != null && NewValue != null; ;
                if (valuesAreEqual)
                {
                    valuesAreEqual = OldValue.Count == NewValue.Count; // Compare the array count

                    foreach (var oldOptionValue in OldValue)
                    {
                        if (valuesAreEqual) // Take Early Exit if there is mismatch is found
                        {
                            var sourceValue = oldOptionValue.Value;
                            var hasMatch = false;
                            foreach (var newOptionValue in NewValue)
                            {
